...
public class CombatUI : MonoBehaviour
{
    ...
    private void HandlePlayerShot(int x, int y, ShotResult result)
    {
        Color color = result == ShotResult.Miss ? Color.blue : Color.red;
        enemyGridButtons[x][y].GetComponent<Image>().color = color;
        enemyGridButtons[x][y].interactable = false;
    }

    private void HandleEnemyShot(int x, int y, ShotResult result)
    {
        Color color = result == ShotResult.Miss ? Color.blue : Color.red;
    ...
    private void OnCellClicked(int x, int y)
    {
        ...
        ShotResult result = GameManager.Instance.FireShot(x, y);
        Debug.Log($"Fired at ({x},{y}): {result}");
    }

    private void RefreshMinimap()
    {
        List<Ship> playerShips = GameManager.Instance.GetShips(0);
        foreach (Ship ship in playerShips)
            foreach (var (cx, cy) in PlacementValidator.GetOccupiedCells(ship))
                minimapCells[cx][cy].color = Color.green;
    }
